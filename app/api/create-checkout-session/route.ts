...
async function resolveLineItem(id: string) {
  if (id.startsWith('price_')) return { price: id, quantity: 1 }
  ...
  throw new Error('Invalid env ID (must start with price_ or prod_)')
}

export async function GET(req: NextRequest) {
  try {
    const plan = (new URL(req.url).searchParams.get('plan') || '').toLowerCase()
